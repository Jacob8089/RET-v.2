...
def new_win():
  # w.destroy()
    ...
    mngc=[ ]
    global rand_data
    rand_data=random.randint(20230101,20300101)
...
#minutely throughput tab_分钟吞吐量选项卡
    ...
    tab2_b2= Button(tab2, text="Import File", command=lambda:threading.Thread(target=open_file_mint).start(), font="Calibri 8 bold", cursor='hand2', width=15, bg=a, fg="white",borderwidth=0)
    ...
    tab2_b1=Button(tab2, text="Reset", command=lambda:res(),font="Calibri 7 bold", width=7, bg="#ff5252", fg="white", borderwidth=0, cursor='hand2')
    ...
    img_btn_tab2= ImageTk.PhotoImage(Image.open(path+"\\images\\RET_up.png"))
    img2_btn_lab=Label(tab2, image=img_btn_tab2, width=20, height=20, borderwidth=0)
    #img2_btn_lab.pack()

    tab2_b3= Button(tab2, image=img_btn_tab2, command=lambda:sample(), width=20, height=20, borderwidth=0, cursor='hand2')
    tab2_b3.place(x=540, y=5)

    tab2_l01=Label(tab2,text='Export',fg='white', bg=a, font="Calibri 5 ")
    tab2_l01.place(x=538, y=25)
    ...
    def open_file_mint():
         ...
         if file:
             ...
             #fig.write_html('minutely.html', auto_open=True)
                 with open((os.path.dirname(str(file_name_path))+"/minthrptgh_log.txt"), 'w') as f:
                 ...
                 tab2_l5.config(text="Done. Click Export button to generate data.")
                 tab2_l6.config(text=" ")
                 messagebox.showinfo("Info", "Data generated. Click Export button to generate data.")
         else:
             tab2_l5.config(text=" ")
             tab2_l6.config(text=" ")
             messagebox.showerror("Alert", "No File Uploaded")   
    ...
    def sample():
         ...
         report_data.to_csv(os.path.dirname(str(file_name_path))+f"\\Minutely_Data{rand}.csv",sep="\t")
